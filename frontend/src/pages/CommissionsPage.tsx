...
export default function CommissionsPage() {
  ...
  const fetchInstances = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || 'Erreur lors du chargement des commissions');
      setInstances([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const addMember = async () => {
    if (!selectedInstanceId) {
      toast.error('Sélectionne une commission');
      return;
    }
    if (!selectedMembreId) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Commissions</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Liste des commissions existantes</p>
        </div>
        <Badge variant="default" className="text-base px-4 py-2">{instances.length} commission(s)</Badge>
      </div>

      {canManageMembers && (
        <Card className="p-0">
          <CardHeader>
            <CardTitle className="text-lg">Ajouter un membre dans une commission (Section)</CardTitle>
          </CardHeader>
